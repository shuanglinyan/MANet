...
class TextExtract(nn.Module):
    ...
    def forward(self, tokens, segments, input_masks):
        ...
        # text_embedding_global = self.embedding_global(caption_id)    # 64, 100, 512
        # text_embedding_global = self.dropout(bert_encoding)
        length = input_masks.sum(1)
        length = length.cpu()
        with torch.no_grad():
            text_feats = self.language_model(tokens, segments, input_masks)
        
        text_embedding_global = self.calculate_different_length_lstm(text_feats, length, self.lstm)
