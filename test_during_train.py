...
def write_result(similarity, img_labels, txt_labels, name, txt_root, best_txt_root, epoch, best):
    ...
    str = "{}--t2i: top1: {:.2%}, top5: {:.2%}, top10: {:.2%}, map: {:.2%}".format(name, t2i_cmc[0], t2i_cmc[4], t2i_cmc[9], t2i_map)
    ...
    if t2i_cmc[0] > best:
        ...
        str = "t2i: top1: {:.2%}, top5: {:.2%}, top10: {:.2%}, map: {:.2%}".format(t2i_cmc[0], t2i_cmc[4], t2i_cmc[9], t2i_map)
        write_txt(str, best_txt_root)
...
def test(opt, epoch, network, img_dataloader, txt_dataloader, best, return_flag=True):
    ...
    for times, [label, caption, caption_length] in enumerate(txt_dataloader):
        (
            tokens,
            segments,
            input_masks,
        ...
        label = label.to(opt.device)
        tokens = tokens.to(opt.device).long()
        segments = segments.to(opt.device).long()
        input_masks = input_masks.to(opt.device).long()

        with torch.no_grad():
            text_global_i, text_local_i = network.txt_embedding(tokens, segments, input_masks)
    ...
    best = write_result(similarity_global, img_labels, txt_labels, 'similarity_glo:',
                        txt_root, best_txt_root, epoch, best)

    best = write_result(similarity_local, img_labels, txt_labels, 'similarity_loc:',
                        txt_root, best_txt_root, epoch, best)
